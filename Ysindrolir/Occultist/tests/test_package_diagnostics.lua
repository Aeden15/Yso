...
local YSO_XML_PATH = join_path(ROOT_DIR, "mudlet packages", "Yso system.xml")
...
do
  ...
  local yso_xml = read_all(YSO_XML_PATH)
  ...
  assert_contains("1v: radiance stage call guarded", yso_xml, "if Yso and Yso.radianceAlert and type(Yso.radianceAlert.fire) == \"function\" then")
  assert_contains("1w: radiance banner call guarded", yso_xml, "if Yso and Yso.radianceAlert and type(Yso.radianceAlert.banner) == \"function\" then")
  assert_contains("1x: pre-tell message cached once", yso_xml, "local msg = (\"%s is setting something up...\"):format(who)")
end
