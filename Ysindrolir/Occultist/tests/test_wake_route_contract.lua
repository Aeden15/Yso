...
local WAKE_PATH = join_path(SCRIPT_DIR, "..", "modules", "Yso", "Core", "wake_bus.lua")
...
do
  ...
  local committed = Q.commit({ route = "group_damage", target = "foe", allow_eqbal = true })
  assert_true("2c: commit success", committed == true)
  local ack = ack_payloads[#ack_payloads] or {}
  assert_eq("2d: eq route_by_lane", type(ack.route_by_lane) == "table" and ack.route_by_lane.eq, "group_damage")
  assert_eq("2e: bal route_by_lane", type(ack.route_by_lane) == "table" and ack.route_by_lane.bal, "group_damage")
  assert_eq("2f: eq target_by_lane", type(ack.target_by_lane) == "table" and ack.target_by_lane.eq, "foe")
...
do
  ...
  Yso.pulse.on_payload_ack({
    route = "magi_focus",
    route_by_lane = { eq = "magi_focus", bal = "group_aff" },
  }, "queue.commit")
  ...
  assert_eq("4e: payload ack second route", calls[2] and calls[2].route or "", "group_aff")
end
