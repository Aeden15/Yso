...
local function _is_party_damage_route()
  ...
  if type(M.party_route) == "function" then
    ...
    -- Modes normalizes "dmg" -> "dam" before storing party_route.
...
local function _select_action(ctx)
  ...
  if type(P.reave_sync_target) == "function" then
    pcall(P.reave_sync_target, tgt, "group_select_action")
  end
  if tgt == "" then
  ...
  -- Aurification execute window outranks Reave and iron finishers.
  ...
  -- Iron remains a route finisher below Aurification/Reave.
