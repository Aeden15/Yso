...
local POLICY_PATH = join_path(SCRIPT_DIR, "..", "Curing", "serverside_policy.lua")
...
local ok3, why3 = P.queue_emergency("eat kelp", { qtype = "bal" })
...
local ok_fail1, why_fail1 = P.queue_emergency("eat kelp", { qtype = "bal" })
local ok_fail2, why_fail2 = P.queue_emergency("eat kelp", { qtype = "bal" })
...
clear_calls()
local ok_q1 = P.queue_emergency("eat kelp", { qtype = "bal" })
assert_true("12a: first queue ok", ok_q1 == true)
local ok_q2 = P.queue_emergency("eat kelp", { qtype = "bal" })
assert_false("12b: second deduped", ok_q2)
P.clear_emergency_dedupe("unit.test.clear")
local ok_q3 = P.queue_emergency("eat kelp", { qtype = "bal" })
...
print("\n=== Test 14: tree readiness bool lifecycle ===")
clear_calls()
P.set_tree_ready(true, "unit.tree.ready")
assert_true("14a: tree ready true", P.tree_ready() == true)
assert_false("14b: touched false when ready true", P.tree_touched() == true)
P.set_tree_touched(true, "unit.tree.touch")
assert_false("14c: touched compatibility flips ready false", P.tree_ready() == true)
P.note_tree_unchanged("unit.tree.unchanged")
assert_false("14d: unchanged is informational only", P.tree_ready() == true)
P.set_tree_ready(true, "unit.tree.ready.2")
assert_true("14e: tree ready restored true", P.tree_ready() == true)
