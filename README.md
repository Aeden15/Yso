...
- **Alchemist Physiology/Formulation boundary cleanup (April 21, 2026)** — Split the humour/evaluate/truewrack support out of `Core/formulation.lua` into `Core/physiology.lua`, leaving `formulation.lua` scoped to Formulation phial skill usage and helper state. Formulation thrown-phial actions now default to `AT GROUND` when no direction is supplied, matching the room-effect use case from the Alchemist skillchart.
- **Occultist canonical module-tree restore (April 21, 2026)** — Restored `Ysindrolir/Occultist/modules/Yso` from the live workspace `Ysindrolir/Yso` tree so test and export-manifest paths resolve again (`../modules/Yso/...`). Re-ran mirror/package sync (`refresh_xml_mirrors.lua`, `rebuild_yso_system_xml.lua`) and validated green (`luac`, 21/21 Lua tests, XML parse, mirror drift 0).
- **Alchemist group damage + Physiology intel route pass (April 21, 2026)** — Added the `alchemist_group_damage` route under the existing `adam` on/off toggle, wired it into the shared route registry/mode loop, and kept lane ownership split between free `evaluate`, direct humour-lane `temper`, and BAL-lane `truewrack`. Expanded Physiology live parsing in `Yso system.xml` with rolled-up pronoun regexes for evaluate, temper, wrack/truewrack, homunculus corrupt, ready lines, exact evaluate vitals, and dirtying lines; evaluate steady counts now gate paralysis legality while live temper success records inferred counts only.
- **Alchemist chart + humour lane + formulation support pass (April 20, 2026)** — Created `Ysindrolir/Alchemist/Alchemical skill_reference chart` directly under the Alchemist folder from the supplied screenshots/text only, added editable Alchemist workspace helpers under `Ysindrolir/Alchemist/Core` for formulation resolution/phial parsing/action building, added workspace trigger handlers for Physiology humour-balance and Formulation `phiallist` lines, mirrored the same support layer into `Ysindrolir/mudlet packages/Yso system.xml`, refactored Formulation aliases to route through centralized helpers instead of hardcoded delivery strings, and widened humour-balance success matching to accept `his` / `her` / `their` / `faes` / `faen`.
...
- **April 20 bug-check fix** — Replaced `Occultist/modules/Yso/Combat/routes/group_damage.lua` placeholder logic with a proper route module (party damage payload planning + Occultist loop echoes), rebuilt `mudlet packages/Yso system.xml`, and re-validated the workspace (`luac`, 20 tests, XML parse, mirror drift).
